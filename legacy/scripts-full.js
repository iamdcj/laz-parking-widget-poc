		...
		return (function (options) {
			...
			LAZWidget.Main = {
				...
				LoadDependencies: function () {
					...
					LAZWidget.Network.GetStyleSheet(WidgetSettings.ContentBase + WidgetSettings.SubPath + WidgetSettings.BrandBase + 'css/LAZSimple-Responsive.css?v=202211110000');
					LAZWidget.Network.GetStyleSheet(WidgetSettings.ContentBase + WidgetSettings.SubPath + WidgetSettings.BrandBase + 'css/LAZSimple.css?v=202210170000');
					LAZWidget.Network.GetStyleSheet(WidgetSettings.ContentBase + WidgetSettings.SubPath + WidgetSettings.BrandBase + 'css/chosen.css?v=202211100000');
				...
				StartWidget: function () {
					...
					if (WidgetSettings.eventdriven === true) {
						...
						//var locationParam = 'eDataLocationId=' + WidgetSettings.LocationIDs.join();
						LAZWidget.Network.GetData('GetEvents', locationParam + "&widgetkey=" + WidgetSettings.WidgetKey + "&eventdriven=" + WidgetSettings.eventdriven, function (result) {
				...
				InvokeRates: function () {
					...
					if (WidgetSettings.eventdriven === false && (WidgetValues.SelectedLID == 0 ||
				...
				BindLocation: function () {
					$.get(WidgetSettings.TemplateBase + 'templates/LAZDefault_Locations.html', function (template) {
						...
						if (WidgetSettings.LocationsJSON.length >= 1) {
							...
							if (WidgetSettings.eventdriven === true) {
								if (WidgetSettings.issoldout === true || $('#park-location').text().includes("Choose a parking location") == true)
						...
						case 'GetLocations':
							for (var i = 0, item; item = Data[i]; i++) {
								if (item.ID != null && item.Name != null) {
									...
									if (LocationInfo.WidgetType !== null) { WidgetSettings.LocationsJSON.push(LocationInfo); };
								}
							}
			...
			LAZWidget.UI = {
					...
					LAZWidget.Helpers.GetDefaultLanguage(firstLoad, function () {
						...
						$.get(WidgetSettings.TemplatePath, function (template) {
							...
							var rendered = Mustache.render(template, {
								Locations: WidgetSettings.LocationsJSON, Events: WidgetSettings.EventsJSON,
								Languages: Language.widgetLanguages, Language: Language.objLang, Header: WidgetSettings.IncludeHeader,
				...
				SetUpEvents: function () {
					$.get(WidgetSettings.TemplateBase + 'templates/LAZDefault_Events.html', function (template) {
						...
						// Render template
						var rendered = Mustache.render(template, { Events: WidgetSettings.EventsJSON });
                       // Set the select element's HTML with the rendered template
						...
						if (WidgetSettings.EventsJSON.length > 0 && WidgetSettings.eventdriven === true) {
				...
				HandleWidgetType: function (WidgetType, stop) {
					...
					if (WidgetSettings.eventdriven === false) {
						...
						if (WidgetSettings.WidgetType.indexOf('EVT') > -1)
							LAZWidget.Network.GetData('GetEvents', locationParam + "&widgetkey=" + WidgetSettings.WidgetKey);
				...
				SetUpMap: function () {
					if (WidgetSettings.UseMap)
						$('#LAZ_Locations_Select .chosen-drop').prepend($('<div>', { id: 'lazGoMapDiv', style: 'height:150px;width:97%;margin:0 1.5% 1.5%;' }));
				},
				//GRSDESK-3583
